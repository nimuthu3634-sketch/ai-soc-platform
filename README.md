...
## Overview
Aegis Core is a university final year project focused on building a professional Security Operations Center platform with AI-assisted workflows. The repository is organized as a TypeScript monorepo so the frontend, backend, shared contracts, and deployment assets can evolve together without tight coupling.
...
- Express/TypeScript backend with JWT auth, role-aware middleware, and standardized API responses
...
- Deployment assets for Vercel frontend hosting and Render or Railway backend hosting
- Optional Docker and GitHub Actions CI setup for local full-stack runs and quality checks
...
The frontend already references the expected logo path and falls back safely if the runtime logo is not available yet.
...
- `FRONTEND_ORIGIN` is still accepted for older local configs, but `FRONTEND_ORIGINS` is preferred for deployment.
...
This separation keeps deployment flexible and makes it easy to swap backend hosting later without changing the frontend architecture.

## Frontend Deployment on Vercel
Use the repository root as the Vercel project root so the frontend build can access the shared `packages/contracts` workspace.
...
- Build: `npm ci && npm run prisma:generate -w @aegis-core/api && npm run build -w @aegis-core/contracts && npm run build -w @aegis-core/api`
...
Do not re-run the demo seed against a live environment that already contains real user or incident data.
...
This is intentionally lightweight so platform logs on Render or Railway remain easy to inspect during demos and reviews.
